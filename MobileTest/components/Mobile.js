...
class Mobile extends React.PureComponent {
    ...
    render() {
      ...
      var headClients = new Array(
        ...
        return <div>
        <button id = 'all' onClick = {()=>this.setClients(1)}>Все</button>
        <button id = 'active' onClick = {()=>this.setClients(2)}>Активные</button>
        <button id = 'block' onClick = {()=>this.setClients(3)}>Заблокированные</button>
        <hr/>
        <table className='Mobile'> 
          <tbody>{tableClients}</tbody>
        </table>
        <button id = 'addClient' onClick = {this.addClient}>Добавить клиента</button>
        {this.addDiv}
      </div>
    }
  }
