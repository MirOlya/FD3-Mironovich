...
function PageFooter(props) {
  ...
  const [maxCounterShowStr, setMaxCounterShowStr] = useState(
    props.begShow === "ALL" ? props.allStrData : Number(props.begShow)
  );
  useEffect(() => {
    setMaxCounterShowStr(
      whatShow === "ALL"
        ? props.allStrData
        : Math.min(props.allStrData, minCounterShowStr + Number(whatShow) - 1)
    );
  ...
  for (let i = 0; i <= arrSelect.length - 1; i++)
    arrSelect[i] = (
      <option key={arrSelect[i]} value={arrSelect[i]}>
        {arrSelect[i]}
      </option>
    );
  ...
  return (
    <div className="footerPage">
      <select
        id="selectWhatShow"
        defaultValue={whatShow}
        onChange={selectWhatShow}
        className="selectWhatShow"
      >
        {arrSelect}
      </select>
      {props.beginRecord != 1 && (
        <Tooltip
          title={
            props.beginRecord === 1
              ? "You are at the top of the list. It is not possible to view the previous lines."
              : "Click to view previous " + props.begShow + " lines."
          }
          enterDelay={500}
          leaveDelay={200}
          placement="bottom-start"
        >
          <button id={"decMinCounter"} onClick={decMinCounter}>
            {"<"}
          </button>
        </Tooltip>
      )}
      <span>{Math.min(props.beginRecord, props.allStrData)}</span>
      <span>{" - "}</span>
      <span>
        {props.begShow === "ALL"
          ? props.allStrData
          : Math.min(props.allStrData, maxCounterShowStr)}
      </span>
      <span>{" из "}</span>
      <span>{+props.allStrData}</span>
      {props.allStrData != maxCounterShowStr && (
        <Tooltip
          title={
            props.allStrData === maxCounterShowStr
              ? "You are at the end of the list. It is not possible to view the following lines."
              : "Click to view following " + props.begShow + " lines."
          }
          enterDelay={500}
          leaveDelay={200}
          placement="bottom-start"
        >
          <button id={"incMinCounter"} onClick={incMinCounter}>
            {">"}
          </button>
        </Tooltip>
      )}
